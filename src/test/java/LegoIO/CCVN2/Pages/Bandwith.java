...
public class Bandwith extends BaseDriver {
	ExtentTest test;
	 public Bandwith(ExtentTest test) {
		 PageFactory.initElements(PageDriver.getCurrentDriver(), this);
		 this.test = test;
	 }
	 
	 @FindBys({
			@FindBy(xpath="//a[@href='/rw/service/bandwidth/home']"),
			
		})
		
	 WebElement Bandwidth;
	 ...
	 WebElement SeeAll;
	 @FindBys({
		 @FindBy(xpath="//a[contains(text(),'OPT visa helpline')]"),
	 ...
	 public void BandwithOption() throws IOException {
		 try {
			 ...
			 test.fail("<p style=\"color:#FF5353; font-size:13px\"><b>Bandwidth option is not working</b></p>");
				Throwable t = new InterruptedException("Exception");
				test.fail(t);
				@SuppressWarnings("unused")
				String dest = GetScreenShot.capture(PageDriver.getCurrentDriver(), "Bandwidth");
				test.fail(MediaEntityBuilder.createScreenCaptureFromPath(dest).build());
				Assert.assertTrue(Bandwidth.isDisplayed());
		 }
	 }
