...
public class Strategy extends BaseDriver {
	ExtentTest test;
	 public Strategy(ExtentTest test) {
		 PageFactory.initElements(PageDriver.getCurrentDriver(), this);
		 this.test = test;
	 }
	 
	 @FindBys({
			@FindBy(xpath="//a[@href='/rw/service/strategy/home']"),
	 ...
	 WebElement SeeAll;
	 @FindBys({
		 @FindBy(xpath="//a[contains(text(),'Give some job tips for getting hired')]"),
		 
	 })
	 
	 WebElement Product;
	 @FindBys({
		 @FindBy(xpath="//input[@value='Top 50 job tips']"),
	 ...
	 public void StrategyOption() throws IOException {
		 try {
			 ...
			 test.fail("<p style=\"color:#FF5353; font-size:13px\"><b>Strategy option is not working</b></p>");
				Throwable t = new InterruptedException("Exception");
				test.fail(t);
				@SuppressWarnings("unused")
				String dest = GetScreenShot.capture(PageDriver.getCurrentDriver(), "Strategy");
				test.fail(MediaEntityBuilder.createScreenCaptureFromPath(dest).build());
				Assert.assertTrue(Strategy.isDisplayed());
		 }
	 }
