...
public class Staircase extends BaseDriver {
	ExtentTest test;
	 public Staircase(ExtentTest test) {
		 PageFactory.initElements(PageDriver.getCurrentDriver(), this);
		 this.test = test;
	 }
	 
	 @FindBys({
			@FindBy(xpath="//a[@href='/rw/service/staircase/home']"),
			
		})
		
	 WebElement staircase;
	 ...
	 WebElement SeeAll;
	 @FindBys({
		 @FindBy(xpath="//a[contains(text(),'Looking for an internship')]"),
		 
	 })
	 
	 WebElement Product;
	 @FindBys({
		 @FindBy(xpath="//input[@value='Challenges']"),
	 ...
	 public void StairCaseOption() throws IOException {
		 try {
			 if(staircase.isDisplayed()) {
			 ...
			 test.fail("<p style=\"color:#FF5353; font-size:13px\"><b>StairCase option is not working</b></p>");
				Throwable t = new InterruptedException("Exception");
				test.fail(t);
				@SuppressWarnings("unused")
				String dest = GetScreenShot.capture(PageDriver.getCurrentDriver(), "StairCase");
				test.fail(MediaEntityBuilder.createScreenCaptureFromPath(dest).build());
				Assert.assertTrue(staircase.isDisplayed());
		 }
	 }
